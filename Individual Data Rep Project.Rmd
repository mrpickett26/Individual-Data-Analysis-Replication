...
library(dotwhisker)
m_yd <- lm(normY_d ~ Size_pathol + Hist_grade + Subtype, data = ans2)
m_y<-lm(Y_d~Size_pathol + Hist_grade + Subtype, data = ans2)
dwplot(list(m_yd, m_y), show_intercept = FALSE) +
            scale_y_discrete(labels=c("Size_pathol" = "Tumor Size (cm)", "Hist_grade" = "Histological Grade",
                        "Subtype" ="Subtype"))+scale_color_hue(labels = c("Normalized y-axis position", "Absolute y-axis position"))+ theme(plot.title = element_text(face="bold"))+ggtitle("Regression Coefficients for Breast Tumor Y-axis positions")
